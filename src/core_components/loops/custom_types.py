...
class StateActionObject(Protocol):
    """
    A StateActionObject is any object with 'store' and 'handler' attributes. The 'store' attribute refers to a StoredStateObject instance for context,
    and the 'handler' attribute refers to a StateHandler instance that can transform and send the object to a State queue.

    Duck Types: StoredStateObject

    """
    store: StoredStateObject | None
...
class StateHandler(EventTransformer, GameLoopObject, Protocol):
    ...
    def handle(self, event: StateActionObject | Any | None) -> bool:
