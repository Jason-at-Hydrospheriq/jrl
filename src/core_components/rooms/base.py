...
class BaseRoom(Protocol):
    ...
    def resize(self, new_size: Tuple[int, int]) -> None:
        ...
        self.upperLeft_corner = MapCoords(self.center.x - self.width // 2, self.center.y - self.height // 2)
        self.lowerRight_corner = MapCoords(self.center.x + self.width // 2, self.center.y + self.height // 2)
