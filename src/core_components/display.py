...
class Display(BaseUI):
    ...
    TILESET = tcod.tileset.load_truetype_font(os.path.join("src","core_components", "widgets", "graphics", "resources", "GoogleSansCode-SemiBold.ttf"), 25, 25)
    ...
    def __init__(self, context: Context | None = None, ui_manifest: UIManifestDict | None = DEFAULT_UI_MANIFEST) -> None:
        ...
        img = Image.open(os.path.join("src","core_components", "widgets", "graphics", "resources", "player", "test-5.png"))
        img = img.convert("RGBA")
        self.TILESET.set_tile(64, np.array(img))
        img = Image.open(os.path.join("src","core_components", "widgets", "graphics", "resources", "mob", "test-3.png"))
        img = img.convert("RGBA")
