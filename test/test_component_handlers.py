...
def test_component_base_handler():
    try:
        ...
        store = GameStore()
        behaviors = set()
        handler = BaseGameHandler(store=store, behaviors=behaviors)
...
def test_component_game_loop_handler():
    try:
        # Arrange
        store = GameStore()
        handler = GameLoopHandler(store=store)
...
# from core_components.loops.base import BaseGameTransformer, EntityStateTableDict, manifest_example
...
#     # Arrange & Act
#     ai = BaseGameTransformer(entity=None, state_table=None)
...
#     # Act
#     ai = BaseGameTransformer(entity=mock_entity, state_table=custom_state_table)
